...
def ermittle_ip_und_broadcast():
    """Gibt lokale IP, zugehörige Broadcast-Adresse und Standard-WHOIS-Port zurück."""
    try:
        with socket.socket(socket.AF_INET, socket.SOCK_DGRAM) as s:
            s.connect(("8.8.8.8", 80))  # Verbindung ins Internet simulieren
            ip = s.getsockname()[0]
...
def config_startup():
    """Lädt entweder eine übergebene oder lokale Konfigurationsdatei."""
    ...
    # Lokale Konfigurationsdatei verwenden
    ...
    if not os.path.exists(config_path):
        print("[INFO] Keine lokale Konfiguration gefunden. Neue wird erstellt.")
...
def datenAufnehmen():
    """Benutzereingaben abfragen und IP-Daten anhängen."""
...
def inConfigSchreiben(login_daten, config_path):
    """Speichert die Benutzerdaten in einer TOML-Datei."""
    try:
        config = {}
...
def get_contacts_path():
    """Pfad zur Kontaktdatei des aktuellen Nutzers."""
    username = getpass.getuser()
...
def check_for_contact_list(contacts_path):
    """Stellt sicher, dass eine Kontaktdatei vorhanden und gültig ist."""
    if not os.path.exists(contacts_path):
        print(f"[INFO] Kontaktliste neu angelegt unter: {contacts_path}")
